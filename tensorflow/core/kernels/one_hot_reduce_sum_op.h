...
#ifndef TENSORFLOW_CORE_KERNELS_ONE_HOT_REDUCE_SUM_OP
#define TENSORFLOW_CORE_KERNELS_ONE_HOT_REDUCE_SUM_OP
...
class OneGeneratorV2 {
 public:
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE
  OneGeneratorV2(const typename TTypes<TI>::ConstMatrix& indices,
      ...
      : indices_(indices), on_value_(on_value), off_value_(off_value) {
        VLOG(0) << "****** Generator Init...";
      }

  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE T
  operator()(const Eigen::array<Eigen::DenseIndex, 3>& pre_depth_suff) const {
    VLOG(0) << "****** pre_depth_suff = {" << pre_depth_suff[0] << "," << pre_depth_suff[1] << "," << pre_depth_suff[2] << "}";
    VLOG(0) << indices_(pre_depth_suff[0], pre_depth_suff[2]);
...
struct OneHotReduceSum {
  EIGEN_ALWAYS_INLINE static void Compute(
      const Device& d, const typename TTypes<TI>::ConstMatrix& indices,
      const typename TTypes<T>::ConstScalar& on_value,
      const typename TTypes<T>::ConstScalar& off_value,
      typename TTypes<T, 3>::Tensor* output) {
    generator::OneGeneratorV2<T, TI> generator(indices, on_value, off_value);
...
#endif /* TENSORFLOW_CORE_KERNELS_ONE_HOT_REDUCE_SUM_OP */
